...
class PortFilterAction(tables.FilterAction):
    def filter(self, table, port, filter_string):
        q = filter_string.lower()

        def comp(port):
            if any([q in (port.name or "").lower()]):
                return True
            return False

        return filter(comp, port)
...
class PortsTable(tables.DataTable):
    ...
    class Meta:
        ...
        table_actions = (PortFilterAction,)
